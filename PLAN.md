...
| KI-012 | Baixa | Parcial | Hierarquia de escala dos bosses: a PROPORÇÃO na arena foi corrigida fiel à lore via `sprite_scale`/offset de pés por cena (Saci premium < Caipora ≈ Curupira < Jesuíta/machados 2.8 ≈ caçador < Boitatá < Mula; contrato em `test_boss_scale_proportions.gd`). **Curupira, Jesuíta, Boitatá, Saci e Mula já migraram** (2026-06): Curupira e Jesuíta no pipeline premium `gen_bosses.py` (canvas 128 arena a escala de nó 1.2 + variante de mapa 48 — leis em `docs/CONCEITO-curupira.md`/`docs/CONCEITO-jesuita.md`, contratos em `test_curupira_sprite_assets.gd`/`test_jesuita_sprite_assets.gd`); Boitatá em `gen_boitata.py` **v2 no traço da protagonista** (2026-07-01: arena 160×128 a escala 1.2, muralha preta-carbonizada + crista de fogo serrilhada encarando a esquerda, set completo idle 5f/windup 3f@15/strike/recover/hurt 2f/death 3f, `.tres` emitido por código, variante de mapa 60×48 — lei em `docs/CONCEITO-boitata.md`, contratos em `test_boitata_sprite_assets.gd`/`test_boitata_sprite_frames.gd`); Saci em `gen_saci.py` (arena 128×128, idle/windup, escala 1.2 — lei em `docs/CONCEITO-saci.md`, contrato em `test_saci_sprite_assets.gd`); Mula em `gen_mula.py` **v3 no traço da protagonista** (2026-07-01: arena 192×192 a escala 0.9, corpo preto-vazio + coroa de fogo serrilhada encarando a esquerda, set completo idle 5f/windup 3f/strike/recover/hurt 2f/death 3f, `.tres` emitido por código, variante de mapa 48×48 — lei em `docs/CONCEITO-mula.md`, contratos em `test_mula_sprite_assets.gd`/`test_mula_sprite_frames.gd`). O redesign do Jesuíta também corrigiu o sprite dele no mapa da Fase 5: `map_enemy.gd` não tinha case `"jesuita"` e o boss final aparecia como o caçador-de-machados (`boss_idle.png`). Resta o caçador-de-machados (48×48 upscalado, texels maiores que os dos comuns) — resolve na sessão de redesign dele. No MAPA os bosses seguem ~48px visuais (Curupira/Jesuíta/Mula/Boitatá via variantes re-renderizadas; só o Saci via clamp interino — ver KI-016). |
...
| KI-016 | Média | Parcial | Os redesigns premium de Saci (PR #61) e Mula (PR #60) repetiram o padrão do KI-015 e ampliaram: (a) `gen_chars.py` seguia chamando `saci()` legado — regenerar sobrescreveria o `saci_idle.png` premium 128 com o 48 antigo (a Mula delegou certo); (b) nenhum dos dois (nem o Boitatá do PR #58) gerou **variante de mapa**: `map_enemy.gd` aplica o `*_idle.png` SEM escala, então Saci 128, Boitatá 160×128 e Mula 192 estouravam o tile de 32px no modo explorar (3–6 tiles de sprite; sem teste cobrindo). Resolvido no merge da PR #59: delegação `gen_saci.generate_all()` + remoção dos legados mortos, e **clamp interino** no `map_enemy.gd` (sprite de boss acima de 64px de altura é reduzido para ~48px visuais via scale do nó). A **Mula pagou a dívida no redesign v3** (2026-07-01): `mula_map.png` 48×48 re-renderizada dos mesmos vetores, `map_enemy.gd` usa a variante dedicada e o clamp não dispara mais para ela. O **Boitatá pagou no redesign v2** (2026-07-01): `boitata_map.png` 60×48 (altura padrão 48, largura preserva a serpente horizontal), base alinhada aos outros bosses de mapa. Pendente: variante de mapa na sessão do Saci, aí removendo o clamp de vez. |
| KI-017 | Baixa | ✅ Resolvida | `test_exploration_phase3.gd::test_phase3_loads_corridor_map_with_fog_and_curupira` era flaky: usava `GameState.start_run()` (seed aleatório) e afirmava nº de inimigos spawnados == `enemy_count`; para certos seeds o mapa não tem tiles alcançáveis suficientes e spawna menos (visto `2 != 5` local e `1 != 5` derrubando o gate do deploy no CI em 2026-07-02). Corrigido fixando `GameState.run_seed = 1` após cada `start_run()` nos testes de exploração com assert de contagem (phase1/phase3/phase4) — seed verificado por varredura: spawna cheio em TODAS as fases. Asserts fortes (`==`) preservados. |
| KI-018 | Média | ✅ Resolvida | **O Cortejo dos Encantados nunca chegou ao combate** e, quando chegou, a mecânica ficou ilegível. (a) A tela "OBTIDO" + o núcleo existiam, mas a integração no turno nunca foi feita — resolvido ligando o golpe ao combate. (b) A 1ª versão usava CARGA (segurar ↑, soltar no cheio): ponto de soltar invisível + segunda metáfora de timing → o jogador não entendia o tempo. **Redesenhado para o "Batuque do Cortejo"** (sequência direcional rítmica, refs Patapon/Guitar Hero/Expedition 33): cada chefe libertado tem um chamado direcional fixo (`Constants.CORTEJO_CALL_FOR_PHASE`: Mula ↑, Boitatá →, Curupira ↓, Saci ←); o turno toca count-in de tambor e uma nota direcional por batida (anel convergente via `TimingSystem`+`TimingBubble`, reusando o tap conhecido + o pipeline dos especiais de boss); acerto invoca a aparição (entra pela direção) + 2 hits; erro hesita, a corrente segue; sequência perfeita → FEVER (acento de maracatu + último golpe crítico). Legibilidade: nota convergente + metrônomo (`AudioDirector.play_cortejo_beat`) + `CortejoBeatTrack` (faixa de pips com preview). Mantidos: `CortejoApparition` + trilha de luz. Aposentados: `HoldTimingSystem`, `ChargeBubble` e constantes de carga. Novo seam testável: `Constants.cortejo_calls_for()`; `TimingSystem.cancel_window()` destrava o `await` por nota no teardown. Cobertura: `test_cortejo_sequence.gd` + `test_cortejo_vfx.gd` + `test_timing_system.gd`. **Pendente (não-bloqueante):** `.wav` dedicados via `gen_sfx.py` (hoje fallback canônico); calibrar `CORTEJO_BEAT_SECS`/janela/FEVER em playtest; `/validate-controls` em device. |
| KI-019 | Média | ✅ Resolvida | O Batuque (KI-018) também foi rejeitado pelo jogador ("não entendi como funciona, sem feedback, mecânica pobre") — direcional é hostil ao **dedão/retrato/navegador** e a mecânica ficava vazia com 1 só espírito após o 1º boss. **Redesenhado (3ª forma) para o "Golpe Perfeito"** (refs Expedition 33/Sekiro/Legend of Dragoon): UMA janela única e apertada com **um toque `ui_up`** (o mesmo do ataque normal — máxima familiaridade pro polegar); **lead-in com slow-mo** telegrafa o golpe; **acerto → BARRAGEM** (todos os chefes libertados desabam de uma vez, dano escala com nº deles, clímax com `spawn_finisher_vfx`+zoom+acento); **erro → contra-ataque** (custo souls, reusa a fórmula canônica de dano do inimigo via `_enemy_counter_damage()` extraída do erro de defesa). Disparo: roll no turno, `CORTEJO_CHANCE = TIMING_DOUBLE_CHANCE`. Aposentados: `CortejoBeatTrack`, `cortejo_calls_for`/`CORTEJO_CALL_FOR_PHASE`/`CORTEJO_BEAT_SECS`/`CORTEJO_COUNT_IN_BEATS`, `play_cortejo_beat`, `test_cortejo_sequence.gd`. Mantidos/reusados: `CortejoApparition` (barragem, `strike(from_left)`), `TimingSystem`/`TimingBubble` (janela única) + `cancel_window`. Novo seam: `Constants.cortejo_spirits_for()`. Cobertura: `test_cortejo_barrage.gd` + `test_cortejo_vfx.gd` + `test_timing_system.gd`. **Pendente:** `.wav` (summon/slam/miss) via `gen_sfx.py`; calibrar `CORTEJO_WINDOW_BASE`/`CORTEJO_MISS_COUNTER_MULT`/teto da barragem em playtest; `/validate-controls` em device. |
| KI-021 | Média | ✅ Resolvida | **DoomFire em build web cobria só a metade esquerda do viewport** (menu + 5 arenas) desde a otimização de grade: `_rebuild` cortava `_cols` pela metade no web sem alargar a célula, deixando uma borda vertical dura no centro da tela — invisível em captura local (`OS.has_feature("web")` falso fora do export) e gritante quando a chama do menu subiu a 90%. Corrigido com células 2× mais LARGAS no web (`pix_x = pix*2`, `_cols = ceil(vp.x/pix_x)`): mesma economia de ~55% por update, cobertura de 100% do width. Seam de teste: membro `_web` (default de parâmetro exigiria expressão constante). Guardado por `test_web_grid_covers_full_width`. |
| KI-020 | Baixa | ✅ Resolvida | `make smoke`/`make gate` podiam crashar o Godot 4.6.3 headless quando stdin vinha de pipe/EOF (`ERROR: Failed to open 'user://logs/...'; signal 11`). Isso quebrou a simulação do hook Codex, que recebe JSON por stdin. Corrigido no harness: comandos Godot headless do `Makefile` agora rodam com `XDG_DATA_HOME=$(GODOT_USER_DATA_DIR)` (default `/tmp/caipora-godot-data`), isolando `user://` de logs/saves do usuário e mantendo `FileAccess.WRITE` funcional; o hook consome JSON e roda `make gate` com stdin fechado. |
| KI-021 | Baixa | Aberta | Detecção de "telefone" por `minf(vp) < Constants.PHONE_SHORT_SIDE_MAX` (640) é **código morto em retrato**: o stretch é `canvas_items`/`expand` com base 750×1334, então `get_visible_rect()` devolve unidades de canvas e o lado curto em retrato é sempre ~750 (medido: janela 393×852 → vp `(750, 1625)`), nunca `< 640`. Consequência: `Constants.particle_amount_scale()` NUNCA corta partículas pela metade no telefone em retrato (o corte de perf da Fase 10 não acontece onde mais importa). O HUD premium (2026-06) contornou usando `is_portrait(vp)` (ratio-invariante) em vez do limiar de px. Fix sugerido: basear o gatilho de telefone em px físicos (`DisplayServer.window_get_size()`/`PixelScale.device_scale`), não no `vp` de canvas. |
