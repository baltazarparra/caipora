...
| KI-018 | Média | ✅ Resolvida | **O Cortejo dos Encantados nunca chegou ao combate.** A tela de unlock "OBTIDO" (`cortejo_unlock_screen.gd`) e o núcleo `HoldTimingSystem` foram entregues (etapa 1 do `docs/CONCEITO-corrente-encantados.md`), mas a etapa 2 (integração no turno) nunca foi feita: `HoldTimingSystem` não era instanciado em lugar nenhum e `_start_caipora_turn` só rolava o ataque duplo. O jogador via "OBTIDO" e o golpe não existia. Resolvido (etapas 2–6 num só PR): roll do Cortejo em `_start_caipora_turn` (precede o duplo), `_start_cortejo_turn`/`_run_cortejo_link`/`_apply_cortejo_hits` com encadeamento, dano 2 hits/elo, killing-blow no meio, teardown (`close_link`); anel de carga `ChargeBubble` (etapa 3); **aparições** `CortejoApparition` — o boss libertado de cada fase retorna como espírito translúcido tingido na aura, reusando seu sprite canônico (etapa 4); **trilha de cortejo** (fio de luz ligando os impactos, etapa 5); **áudio** `AudioDirector.play_cortejo_*`/`set_cortejo_active` — charge loop com pitch, trinco no armado, stinger por espírito, perdido, full-chain accent, STEM_TOP + duck (etapa 6, com fallback aos sons canônicos; dormente até os .wav dedicados). Texto da tela passou a ensinar "SEGURE ↑ / SOLTE no cheio". Cobertura: `test_hold_timing_system.gd` (núcleo) + `test_cortejo_vfx.gd` (VFX/mapeamento). **Pendente (asset, não-bloqueante):** gerar os .wav dedicados do Cortejo (charge/armed/4 espíritos/miss/full) via `gen_sfx.py` — hoje os hooks caem nos fallbacks canônicos. Validação interativa `/validate-controls` (segurar ↑ no touch) pendente de teste em device. |
