...
## Roteamento de saída da tela de fim (WIN / GAME_OVER). Regra: fim de run NÃO leva ao
## acampamento — o hub só entra ENTRE fases (advance_phase_via_hub). Vitória e derrota voltam
## ao MENU PRINCIPAL. _dismiss_target() é puro, então a instância roda DESTACADA da árvore
## (sem _ready, sem exigir os $nós-filhos).
...
func test_win_goes_to_main_menu() -> void:
	assert_eq(_make(true)._dismiss_target(), SignalBus.Screen.MAIN_MENU,
		"vitória volta ao menu principal")
