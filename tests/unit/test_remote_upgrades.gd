...
# através dos helpers públicos de MetaProgression (_upgrade_cost/_upgrade_attr).
...
func test_cost_override_wins_over_baked() -> void:
	...
	assert_eq(MetaProgression._upgrade_cost("forca"), 99, "custo do override vence o baked")
...
func test_empty_override_falls_back_to_upgrade_defs() -> void:
	RemoteUpgrades._set_overrides_for_test({})
	assert_eq(MetaProgression._upgrade_cost("forca"), 5, "sem override → custo baked (5)")
	assert_eq(MetaProgression._upgrade_attr("forca"), 1, "sem override → dmg baked (1)")
	assert_eq(MetaProgression._upgrade_cost("saude"), 6, "sem override → custo baked saude (6)")
...
func test_override_isolated_per_key() -> void:
	...
	assert_eq(MetaProgression._upgrade_cost("forca_2"), 10, "override de forca não afeta forca_2 (custo)")
