...
25. **O viewport do `make test` headless é QUADRADO (1334×1334), nunca retrato.** Apesar do `project.godot` declarar 750×1334, o headless abre janela 100×100 e o stretch `canvas_items`+`expand` resolve um visible_rect quadrado — `Constants.is_portrait()` retorna `false` em teste. Comportamento dependente de orientação (ex.: âncoras das caixas do `DialogueScreen`) precisa de um seam puro testável (`_apply_box_anchors(portrait)`) em vez de asserts que dependem do viewport real. Validação visual de retrato/paisagem é por captura (`preview_boss_text.gd`, `screenshot.gd` com `--resolution WxH`). Bônus: rótulo de `Label` SEM autowrap dentro de `VBoxContainer` infla a largura mínima do container além da caixa ancorada e arrasta TODO o conteúdo para fora do viewport — texto dinâmico (nomes de chefe) em caixa estreita exige `autowrap_mode`.
