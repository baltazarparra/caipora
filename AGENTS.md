...
18. **Golpe Perfeito (ex-Cortejo) é uma janela única em `ui_up`.** O Cortejo antigo de SEGURAR→SOLTAR foi aposentado; a forma atual é o **Golpe Perfeito** descrito em `PLAN.md` KI-019. Disparo: roll no turno (`CORTEJO_CHANCE = TIMING_DOUBLE_CHANCE`), lead-in com slow-mo, uma janela apertada no mesmo input familiar do ataque normal (`ui_up`). Acerto invoca a barragem dos chefes libertados (`Constants.cortejo_spirits_for()`); erro abre contra-ataque com dano calculado pela fórmula canônica do inimigo. Não reintroduza hold, carga, sequência direcional, `CortejoBeatTrack`, `cortejo_calls_for`, `CORTEJO_BEAT_SECS` ou `play_cortejo_beat`. Mexeu nisso? `/validate-controls` + `make gate`.
