...
24. **Deploy do Pages: o que está no ar é o que o CI builda, e a fila engasga.** O workflow (`.github/workflows/deploy.yml`) monta `site/` na raiz + um `make export` FRESCO em `/play/` — o `export/` commitado no repo NÃO é servido pelo Pages (existe para o zip do itch e histórico). Diagnóstico de deploy: (a) run preso em `deployment_queued` = fila do Pages do lado do GitHub, não bug do repo — cheque o componente Pages em `https://www.githubstatus.com/api/v2/summary.json`; o job tem timeout de 20 min/tentativa + 1 retry; (b) para re-rodar só o job morto use `gh api -X POST repos/baltazarparra/caipora/actions/runs/<id>/rerun-failed-jobs` (o `gh` local é antigo, sem `run rerun --failed`); (c) o checkout do job `build` precisa de `fetch-depth: 0` — raso, o `git rev-list --count HEAD` do `make export` conta 1 e todo build publicado sai `alpha-X.Y.1` (foi o sintoma que mascarou um deploy velho em 2026-07-02); (d) `cancel-in-progress: false` é intencional (PR #87 — pushes rápidos se auto-cancelavam); (e) o service worker do PWA cacheia agressivo: valide o live com query-string (`?v=...`) ou aba limpa.
