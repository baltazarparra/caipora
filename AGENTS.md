...
25. **Modo HD (qualidade gráfica) é flag com reload, default por dispositivo.** O flag vive em `scripts/utils/quality.gd` (`Quality` — classe ESTÁTICA, não autoload: só usa ConfigFile/OS/DisplayServer/JavaScriptBridge e por isso compila sob `-s`, ao contrário de classes que referenciam autoloads — gotcha #14). Precedência de `hd_enabled()`: **(1)** `?hd=0|1` na URL (web) — autoridade do boot, porque o `user://` no web é IndexedDB com syncfs ASSÍNCRONO e o cfg salvo instantes antes do reload pode não persistir; o valor da URL é re-semeado no cfg; **(2)** `user://settings.cfg [video] hd`; **(3)** default por dispositivo: desktop (nativo/web), qualquer Apple ou sem touchscreen (teclado) = ON; Android/touch genérico = OFF. O botão **HD** (menu, à esquerda do speaker — `HdButton`) abre o `HdConfirmPanel` (Confirmar/Cancelar explícitos), que persiste ANTES do fade e recarrega navegando com `?hd=` mesclado via `URLSearchParams` (NUNCA `reload()` puro; preserva `dpr`/`perf`); nativo/editor volta ao menu. **O que o HD liga:** `ParticleRim` (contorno de brasas na silhueta via `EMISSION_SHAPE_DIRECTED_POINTS` sobre pontos de borda extraídos do alpha do frame idle, cache estático por textura — Caipora em laranja-juba esquentando por tier de Fúria/CHAMA + faísca verde rara tier ≥ 3; inimigos via `Criatura._rim_color()`/`MapEnemy._aura_color()`, comuns em `COLOR_RIM_ENEMY` FORA da paleta da protagonista), rastro de brasas + ghost frames de fogo do Boitatá (arena e mapa), e `Constants.particle_amount_scale()` = **1.0 em qualquer device** (o que também abre o gate do front-light per-ator no `ActorContrast`). HD OFF = jogo exatamente como antes (componentes nem instanciam). **Testes:** todo teste sensível a HD seta `Quality._set_for_test()` em `before_each` e `_reset_for_test()` em `after_each` — em CI linux o default é TRUE, nunca assumir false. **Previews `-s`:** `particle_rim.gd` e cenas cujos scripts o referenciam carregam via `load()` em runtime (referencia `MetaProgression` — gotcha #14); ver `preview_particle_rim.gd`/`preview_boitata_trail.gd`.
