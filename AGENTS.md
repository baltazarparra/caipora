...
24. **Deploy do Pages: o que está no ar é o que o CI builda, e a fila engasga.** O workflow (`.github/workflows/deploy.yml`) monta `site/` na raiz + um `make export` FRESCO em `/play/` — o `export/` commitado no repo NÃO é servido pelo Pages (existe para o zip do itch e histórico). Diagnóstico de deploy: (a) run preso em `deployment_queued` = fila do Pages do lado do GitHub, não bug do repo — cheque o componente Pages em `https://www.githubstatus.com/api/v2/summary.json`; o `actions/deploy-pages` CLAMPA o timeout em 10 min/tentativa e o **ID do deployment do Pages é o SHA do commit** — retry NUNCA como 2º step no MESMO job (o cleanup de falha da 1ª tentativa cancela por SHA e mata o deployment da retry em segundos, "Deployment cancelled."); a retry é o job separado `deploy_retry` (`needs: deploy`, `if: always() && needs.deploy.result == 'failure'`); (b) para re-rodar só o job morto use `gh api -X POST repos/baltazarparra/caipora/actions/runs/<id>/rerun-failed-jobs` (o `gh` local é antigo, sem `run rerun --failed`); (c) o checkout do job `build` precisa de `fetch-depth: 0` — raso, o `git rev-list --count HEAD` do `make export` conta 1 e todo build publicado sai `alpha-X.Y.1` (foi o sintoma que mascarou um deploy velho em 2026-07-02); (d) `cancel-in-progress: false` é intencional (PR #87 — pushes rápidos se auto-cancelavam); (e) o service worker do PWA cacheia agressivo: valide o live com query-string (`?v=...`) ou aba limpa; (f) erro 400 `due to in progress deployment. Please cancel <sha> first` = deployment encravado no backend do Pages bloqueando a fila — cheque `gh api repos/baltazarparra/caipora/pages/deployments/<sha>`: se `{"status":"succeed"}` o conteúdo dele JÁ subiu (só re-rode os jobs mortos do run bloqueado); se ainda pendente, `gh api -X POST .../pages/deployments/<sha>/cancel` e re-rode.

25. **Modo HD (qualidade gráfica) é flag com reload, default por dispositivo.** O flag vive em `scripts/utils/quality.gd` (`Quality` — classe ESTÁTICA, não autoload: só usa ConfigFile/OS/DisplayServer/JavaScriptBridge e por isso compila sob `-s`, ao contrário de classes que referenciam autoloads — gotcha #14). Precedência de `hd_enabled()`: **(1)** `?hd=0|1` na URL (web) — autoridade do boot, porque o `user://` no web é IndexedDB com syncfs ASSÍNCRONO e o cfg salvo instantes antes do reload pode não persistir; o valor da URL é re-semeado no cfg; **(2)** `user://settings.cfg [video] hd`; **(3)** default por dispositivo: desktop (nativo/web), qualquer Apple ou sem touchscreen (teclado) = ON; Android/touch genérico = OFF. O botão **HD** (menu, à esquerda do speaker — `HdButton`) abre o `HdConfirmPanel` (Confirmar/Cancelar explícitos), que persiste ANTES do fade e recarrega navegando com `?hd=` mesclado via `URLSearchParams` (NUNCA `reload()` puro; preserva `dpr`/`perf`); nativo/editor volta ao menu. **Orçamento (v2 "Impressionante"):** HD off mira 60fps (jogo byte a byte como antes — componentes nem instanciam); HD on aceita ~30fps por CARGA, **SEM cap** de `Engine.max_fps` (decisão do dono; medição no export desktop: 60→59fps, med 16.8→17.4ms, draw calls EMPATADOS em 163 — o batching do `ADDITIVE_MATERIAL` único absorve as partículas extras; o custo real é CPU/fillrate, que aparece em mobile). **O que o HD liga:** `ParticleRim` v2 (brasas da silhueta via `EMISSION_SHAPE_DIRECTED_POINTS` + **RimHalo** bloom fake com `ForestLight.LIGHT_TEXTURE` + **RimLight** PointLight2D pulsante + **flare()** no PERFEITO — pico de luz + RimBurst + anel de choque, conectado a `attack/defense_result_perfect` — + **FootEmbers** na passada; Caipora em laranja-juba esquentando por tier de Fúria/CHAMA, faísca verde rara tier ≥ 3; luz/halo SÓ em Caipora + ator de arena + boss/miniboss/espírito — comuns do mapa ficam nas brasas, `COLOR_RIM_ENEMY` FORA da paleta da protagonista), **coroa orbital** (`ParticleRim.attach_crown`, `local_coords=TRUE` senão fica para trás no teleporte por tile), **brasa de libertação** (`GameState.last_defeated_enemy_id`, volátil), **arena viva** (`set_combat_mode` não culla em HD; DoomFire segue pausado SEMPRE; brasas ambientes em toda fase com ramp da paleta), **santuário amplificado** (CampSpirit rim/coroa/glow pulsante; pira/fátuos/vento ×2), afterimages dobradas tintadas de fogo, e `Constants.particle_amount_scale()` = **1.0 em qualquer device** (abre o gate do front-light per-ator). **Densidade:** emissores AMBIENTES usam `Constants.ambient_amount_scale()` (= particle_amount_scale × `Quality.heavy()`, 2× em HD) ou `×Quality.heavy()` quando o amount era fixo; one-shots de combate (FeedbackSystem) ficam em `particle_amount_scale` — feedback de timing NÃO engorda com HD. Energias/escalas por modo usam `Quality.pick(leve, hd)`. **Testes:** todo teste sensível a HD seta `Quality._set_for_test()` em `before_each` e `_reset_for_test()` em `after_each` — em CI linux o default é TRUE, nunca assumir false. **Previews `-s`:** `particle_rim.gd` e cenas cujos scripts o referenciam carregam via `load()` em runtime (referencia `MetaProgression`/`SignalBus` — gotcha #14); ver `preview_particle_rim.gd`/`preview_boitata_trail.gd`/`preview_boss_crown.gd`. `screenshot.gd --run` inicia a run antes da cena (arena/exploração sem run matam a Caipora no frame 1).
